...
import { DocumentReview, Document, Deliverable, User } from '../models';

// Create a review request
export const createReviewRequest = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (itemType === 'Chapter') {
      await Document.findByIdAndUpdate(itemId, { status: 'InReview' });
...
export const getReviewsByProject = async (req: AuthRequest, res: Response) => {
  try {
    const reviews = await DocumentReview.find({ project: req.params.projectId })
...
export const getReviewsForReviewer = async (req: AuthRequest, res: Response) => {
  try {
    const reviews = await DocumentReview.find({ reviewer: req.user._id })
      .populate('requestedBy', 'name')
...
export const submitReviewVerdict = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (review.itemType === 'Chapter') {
      ...
      let docStatus: 'Draft' | 'InReview' | 'Approved' = 'Draft';
      if (status === 'Approved') docStatus = 'Approved';
      else if (status === 'InReview') docStatus = 'InReview';
      
      await Document.findByIdAndUpdate(review.itemId, { status: docStatus });
    } else if (review.itemType === 'Deliverable') {
      // Deliverable statuses: 'Pending' | 'InReview' | 'Approved' | 'ChangesRequested' | 'Finalized'
      let delStatus: 'Pending' | 'InReview' | 'Approved' | 'ChangesRequested' = 'Pending';
      if (status === 'Approved') delStatus = 'Approved';
      else if (status === 'ChangesRequested') delStatus = 'ChangesRequested';
      else if (status === 'Rejected') delStatus = 'Pending';
      else if (status === 'InReview') delStatus = 'InReview';

      await Deliverable.findByIdAndUpdate(review.itemId, { status: delStatus });
    }
