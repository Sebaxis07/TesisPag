...
export const getRequirementsByProject = async (req: ProjectAuthRequest, res: Response) => {
  try {
...
export const getRequirementById = async (req: ProjectAuthRequest, res: Response) => {
  try {
    ...
    const role = await getProjectRole(req.user._id, requirement.project.toString());
...
export const updateRequirement = async (req: ProjectAuthRequest, res: Response) => {
  try {
    const requirement = await Requirement.findById(req.params.id);
    if (!requirement) {
      return res.status(404).json({ message: 'Requirement not found' });
...
export const deleteRequirement = async (req: ProjectAuthRequest, res: Response) => {
  try {
    const requirement = await Requirement.findById(req.params.id);
    if (!requirement) {
      return res.status(404).json({ message: 'Requirement not found' });
...
export const extractRequirementsFromText = async (req: ProjectAuthRequest, res: Response) => {
  try {
    ...
    await logAudit(
      req,
      projectId,
      'SUGGEST_REQUIREMENTS_AI',
      'Requirement',
      'Multiple',
      `Requested AI requirement suggestions. Received ${aiRequirements.length} suggestions`
    );
...
export const createRequirementsBulk = async (req: ProjectAuthRequest, res: Response) => {
  try {
    ...
    await logAudit(
      ...
      'Requirement',
      'Multiple',
      `Created/Updated ${savedRequirements.length} requirements from AI suggestions`
    );
