...
exports.submitADRReview = exports.getADRReviews = exports.submitADRForReview = exports.deleteADR = exports.updateADR = exports.getADRById = exports.getADRsByProject = exports.createADR = void 0;
...
const getADRsByProject = async (req, res) => {
    try {
        ...
        const adrs = await models_1.ADRDecision.find({ project: projectId }).sort({ code: 1 });
...
exports.getADRsByProject = getADRsByProject;
// 3. Get ADR by ID
const getADRById = async (req, res) => {
    try {
        const adr = await models_1.ADRDecision.findById(req.params.id);
        if (!adr) {
            return res.status(404).json({ message: 'ADR not found' });
        }
        const role = await (0, auth_1.getProjectRole)(req.user._id, adr.project.toString());
...
const updateADR = async (req, res) => {
    try {
        ...
        if (oldStatus === 'ChangesRequested') {
            ...
            await models_1.ADRReview.deleteMany({ adr: adr._id });
            adr.currentApprovals = 0;
        }
        await adr.save();
        await (0, auditLogger_1.logAudit)(req, adr.project.toString(), 'UPDATE_ADR', 'ADRDecision', adr._id.toString(), `Code: ${adr.code}, Title: ${adr.title} (v${adr.version})`);
        return res.json(adr);
    }
    catch (error) {
        return res.status(500).json({ message: error.message });
...
const deleteADR = async (req, res) => {
    try {
        ...
        await models_1.ADRDecision.findByIdAndDelete(req.params.id);
        await models_1.ADRReview.deleteMany({ adr: adr._id }); // Cascade delete reviews
        await (0, auditLogger_1.logAudit)(req, adr.project.toString(), 'DELETE_ADR', 'ADRDecision', adr._id.toString(), `Code: ${adr.code}, Title: ${adr.title}`);
...
exports.deleteADR = deleteADR;
// 6. Submit ADR for review
const submitADRForReview = async (req, res) => {
    try {
        const adr = await models_1.ADRDecision.findById(req.params.id);
        if (!adr) {
            return res.status(404).json({ message: 'ADR not found' });
        ...
        const members = await models_1.TeamMember.find({ project: adr.project }).populate('user');
        const otherMembers = members.filter(m => m.user && m.user._id.toString() !== req.user._id.toString());
        ...
        await (0, auditLogger_1.logAudit)(req, adr.project.toString(), 'SUBMIT_ADR_REVIEW', 'ADRDecision', adr._id.toString(), `Code: ${adr.code} submitted for team review.`);
        return res.json(adr);
    }
    catch (error) {
        return res.status(500).json({ message: error.message });
...
exports.submitADRForReview = submitADRForReview;
// 7. Get reviews for an ADR
const getADRReviews = async (req, res) => {
    try {
        const reviews = await models_1.ADRReview.find({ adr: req.params.id }).sort({ createdAt: 1 });
...
exports.getADRReviews = getADRReviews;
// 8. Submit Review (Approve, Reject, Request Changes)
const submitADRReview = async (req, res) => {
    try {
        ...
        let review = await models_1.ADRReview.findOne({ adr: adr._id, reviewer: req.user._id });
        if (review) {
            ...
            review = await models_1.ADRReview.create({
                adr: adr._id,
        ...
        const allReviews = await models_1.ADRReview.find({ adr: adr._id });
        ...
        if (oldStatus !== adr.status) {
            const ownerUser = await models_1.User.findById(adr.owner);
            if (ownerUser) {
        ...
        await (0, auditLogger_1.logAudit)(req, adr.project.toString(), 'SUBMIT_REVIEW', 'ADRDecision', adr._id.toString(), `Review: ${decision} by ${req.user.name} for ${adr.code}. Result state: ${adr.status}.`);
...
exports.submitADRReview = submitADRReview;
