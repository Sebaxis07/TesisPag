...
exports.recalculateRequirementStatus = recalculateRequirementStatus;
exports.recalculateRequirementsForTask = recalculateRequirementsForTask;
 ...
 */
async function recalculateRequirementStatus(requirementId) {
    try {
        const requirement = await models_1.Requirement.findById(requirementId).populate('linkedTasks');
        if (!requirement)
            return;
        ...
        const tasks = requirement.linkedTasks;
 ...
 */
async function recalculateRequirementsForTask(taskId) {
    try {
        ...
        const reqIds = Array.from(new Set(links.map(l => l.sourceType === 'Requirement' ? l.sourceId.toString() : l.targetId.toString())));
